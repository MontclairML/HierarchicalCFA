...
Cooperation and **engageable** are significant predictors of OCBs. R-square is 0.52. 
...
Concern for others, **mindful**, **principled**, **though mind** and **hexaco** are all significant predictors of CWBs.R-square is 0.58
...
**Mindful**, and hexaco are significant predictors of task performance. R-square is 0.46. 
```{r}
...
Assertive, Influencing, initiative, **stress tolerance**, mindful, **engageable**, **though mind** and hexaco are significant predictors of intention to leave.R-square is 0.47
```{r}
...
**Achievement oriented**, **adaptable**, **analytical thinking**, **energetic**, **concern for others**, **multitasking**, mindful, **principled**, and **engageable** are all significant predictors of engagement.R-square is 0.93, this might be a redundant scale. 
 ...
 **stress tolerance**, mindful, **principled**, **though mind**, are significant predictors of incivility. R-square is 0.42 
...
Assertive, creative, influencing, mindful, and engageable are significant predictors of idk.R-square is 0.4 
...
Hex=~Sincerity+Patience+Hexaco
'

fit1<-cfa(HS.Model, data=final_data)#FIT MODEL TO DATA FROM ROUND 1
...
extraversion<-data.frame(Assertive, Competitive, Energetic, Influencing, Initiative, Persistence, Sociability, Though_mind)
library(psych)
alpha(extraversion)
EnergeticCITC<-select(rawdata_df[,26:n],starts_with(c("Nrg"), ignore.case=FALSE))
alpha(EnergeticCITC)
rawdata_df$Nrg14MN<-NULL
rawdata_df$Nrg17MN<-NULL
rawdata_df$Nrg9MN<-NULL
rawdata_df$Nrg20TN<-NULL

conscientiousness<-data.frame(Achievement_Oriented, Dependability, Detail_Oriented, Methodical, Rule_Follower)
alpha(conscientiousness)
rule_followerCITC<-select(rawdata_df[,26:n],contains(c("Cmpl")))
alpha(rule_followerCITC)
rawdata_df$Cmpl5MP<-NULL
rawdata_df$Cmpl19MP<-NULL
rawdata_df$Cmpl3TN<-NULL
rawdata_df$Cmpl16TP<-NULL

neuroticsm<-data.frame(Adaptable, Optimism, Self_control, Stress_Tolerance)
alpha(neuroticsm)
adaptableCITC<-select(rawdata_df[,26:n],contains(c("Adpt")))
alpha(adaptableCITC)
rawdata_df$Adpt18MN<-NULL
rawdata_df$Adpt19TN<-NULL
rawdata_df$Adpt16TP<-NULL
rawdata_df$Adpt9MN<-NULL

other<-data.frame(Principled, Multitasking, Mindful)
alpha(other)
multitaskCITC<-select(rawdata_df[,26:n],contains(c("Multi")))
alpha(multitaskCITC)
rawdata_df$Multi20TN<-NULL
rawdata_df$Multi21TN<-NULL
...
```{r}
correlation2<-cor(final_data[1:31])
write.csv(correlation2, "correlations2.csv")
library(corrplot)
corrplot(correlation2, method="number")
factanal.none <- factanal(final_data[1:31], factors=7, scores = c("regression"), rotation = "varimax")
print(factanal.none)
write.csv(factanal.none$loadings, "factor_analysis.csv")
```
...
hexaco<-select(rawdata_df[,26:n],contains(c("Hex")))
hexaco<-data.frame(hexaco, final_data)%>%
  select(1:90)%>%
  mutate(HexAggr=rowMeans(hexaco[1:60]))
...
hexaco<-data.frame(hexaco, hexHone, hexOpen, hexCons, hexAgree, hexExtra, hexEmo)

agreeableness<-final_data%>%select(Team_Oriented, Cooperation, Concern_for_Others)%>%rowMeans()
conscientiousness<-final_data%>%select(Achievement_Oriented, Dependability, Detail_Oriented, Methodical, Rule_Follower)%>%rowMeans()
openess<-final_data%>%select(Learning_Orientation, Creative, Analytical_Thinking)%>%rowMeans()
extraversion<-final_data%>%select(Assertive, Energetic, Competitive, Initiative, Influencing, Persistence)%>%rowMeans()
neuroticism<-final_data%>%select(Adaptable, Stress_Tolerance, Self_control, Optimism)%>%rowMeans()
multi<-final_data%>%select(Engageable, Mindful, Principled, Multitasking)%>%rowMeans()

library(corrr)
hexaco<-data.frame(hexaco, agreeableness, conscientiousness, openess, extraversion, neuroticism, multi)
hexaco<-hexaco%>%select(61:72)
hexaco%>%correlate()%>%shave()%>%fashion
...
```{r hexacon regression}

hexReg<-final_data%>%select(1:31)
hexReg<-lm(Hexaco~., data=hexReg)
summary(hexReg)
